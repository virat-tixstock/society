...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item" aria-current="expense"> {{ __('Create') }}</li>
@endsection
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="expense_type" value="Utility"
                                        @if ($expense->expense_type == 'Utility') checked @endif id="utility">
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="expense_type" value="Common"
                                        @if ($expense->expense_type == 'Common') checked @endif id="common">
                ...
                <div class="card-body">
                    @foreach ($expense->ExpenseDetails as $item)
                        <div class="row location_list">
                            {!! Form::hidden('expense_id[]', $item->id, []) !!}
                            <div class="form-group col-md-4 col-lg-4">
                                {{ Form::label('bill_type', __('Bill Type'), ['class' => 'form-label']) }}
                                {!! Form::select('bill_type[]', $bill_types, $item->bill_type, [
                                    'class' => 'form-control select2',
                                ]) !!}
                            </div>
                            <div class="form-group col-md-3 col-lg-3">
                                {{ Form::label('amount', __('amount'), ['class' => 'form-label']) }}
                                {{ Form::text('amount[]', $item->amount, ['class' => 'form-control', 'placeholder' => __('Enter amount')]) }}
                            </div>
                            <div class="form-group col-md-4 col-lg-4">
                                {{ Form::label('note', __('note'), ['class' => 'form-label']) }}
                                {{ Form::textarea('note[]', $item->note, ['class' => 'form-control', 'rows' => '1', 'placeholder' => __('Enter note')]) }}
                            </div>
                    ...
                    @endforeach
                    @if (count($expense->ExpenseDetails) == 0)
                        <div class="row location_list">
                            <div class="form-group col-md-4 col-lg-4">
                                {{ Form::label('bill_type', __('Bill Type'), ['class' => 'form-label']) }}
