                        ...
                        <div class="row g-3">
                            <div class="col-12">
                                <div class="row align-items-center g-3">
                                    <div class="col-sm-6">
                                        ...
                                        <p class="mb-0">{{ maintenancePrefix() . $maintenance->maintenance_id }}</p>
                                    </div>
                                    <div class="col-sm-6 text-sm-end">
                                        <h6>
                                            {{ __('Month') }}
                                            <span class="text-muted f-w-400">{{ monthList()[$maintenance->month] }}</span>
                                        </h6>
                                        <h6>
                                            {{ __('Status') }}
                                            @if ($maintenance->status == 'Unpaid')
                                                <span class="badge text-bg-danger">{{ $maintenance->status }}</span>
                                            @else
                                                <span class="badge text-bg-success">{{ $maintenance->status }}</span>
                                            @endif
                            ...
                            <div class="col-sm-6">
                                <div class="border rounded p-3">
                                    ...
                                    {{-- <h5>{{ !empty($maintenance->Building) ? $maintenance->Building->name : '' }}</h5> --}}
                                    <h5>{{ !empty($maintenance->Member) ? $maintenance->Member->name : '' }}</h5>
                                    ...
                                    <table class="table table-hover mb-0">
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($maintenance->Details as $detail)
                                                ...
                                                @endphp
                                                <tr>
                                                    <td>{{ !empty($detail->MaintenanceType) ? $detail->MaintenanceType->title : '-' }}
                                                    </td>
